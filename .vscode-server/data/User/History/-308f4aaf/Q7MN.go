...
func signingHandler(w http.ResponseWriter, r *http.Request){
	...
	if err != nil {
		...
		fmt.Printf("Failed to generate toekn %v\n", err)
		return
	}

	w.Write([]byte(toeknString))
}
...
func signupHandler(w http.ResponseWriter, r *http.Request){
	...
	var user model.User
	if err := decoder.Decode(&user); err != nil {
		http.Error(w, "Cannot decode user data fromn client", http.StatusBadRequest)
		fmt.Printf("Cannot decode user data from client %v\n", err)
		return
	}
	...
	if user.Username == "" || user.Password == "" || regexp.MustCompile(`^[a-z0-9]$`).MatchString(user.Username){
		...
		fmt.Printf("Invalid useranme or password\n")
		return
	}
